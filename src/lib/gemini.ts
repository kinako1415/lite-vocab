...
1. 単語の抽出について：
   ...
   - サイト内のすべての単語を抽出してください（制限なし）
   - 最低でも30個以上の単語を抽出することを目標としてください

2. 抽出の基準：
   - 学習に値する重要な単語を抽出してください
   - 一般的な使用頻度の高い単語を優先してください
   - 文法的に重要な単語（例：助動詞、前置詞など）も含めてください
   - サイト内の単語を漏れなく抽出してください
   - 単語の抽出数に制限を設けないでください
...
4. 出力形式：
   ...
   - レスポンスのサイズ制限に達するまで、可能な限り多くの単語を抽出してください

5. 特殊なケース：
   - 動詞の場合は、基本的な形（原形）を抽出してください
   - 名詞の場合は、単数形を基本とし、必要に応じて複数形も含めてください
   - 形容詞の場合は、基本的な形を抽出し、比較級や最上級の情報も含めてください
   - 同義語や関連語も可能な限り含めてください

6. 重要な指示：
   - 単語の抽出数に制限を設けないでください
   - サイト内のすべての単語を抽出することを目標としてください
   - 最低でも30個以上の単語を抽出してください
   - 可能な限り多くの単語を抽出してください
`;
